...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Função para exibir feriados do mês atual
    function exibirFeriadosDoMes(feriados) {
        const feriadosMesContainer = document.getElementById('feriados-mes-container');
        
        if (!feriadosMesContainer) {
            console.error('Container de feriados não encontrado');
            return;
        }

        // Limpar container
        feriadosMesContainer.innerHTML = '';
        ...
        const feriadosDoMes = feriados.filter(feriado => {
            const [dia, mes, ano] = feriado.data.split('/').map(Number);
            return mes === mesAtual && ano === anoAtual;
        });

        // Ordenar feriados por data
        feriadosDoMes.sort((a, b) => {
            const [diaA, mesA, anoA] = a.data.split('/').map(Number);
            const [diaB, mesB, anoB] = b.data.split('/').map(Number);
            return new Date(anoA, mesA - 1, diaA) - new Date(anoB, mesB - 1, diaB);
        });

        // Criar elementos para cada feriado
        feriadosDoMes.forEach(feriado => {
            ...
            feriadosMesContainer.appendChild(feriadoElemento);
        });

        // Se não houver feriados, mostrar mensagem
        if (feriadosDoMes.length === 0) {
            feriadosMesContainer.innerHTML = '<p>Nenhum feriado este mês</p>';
        }
    }

    // Função para verificar se uma data é feriado
    function verificarFeriado(data, feriados) {
        const dataFormatada = formatarData(data);
        const feriadoEncontrado = feriados.find(feriado => feriado.data === dataFormatada);
        return feriadoEncontrado ? feriadoEncontrado : null;
    }
